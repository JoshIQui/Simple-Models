...
const setDogName = async (req, res) => {
  // Check if the parameters have been filled out
  ...
  try {
    /* newDog is a version of our newDog that is database-friendly. If you print it, you will
    ...
    */
    ...
    /* If something goes wrong while communicating with the database, log the error and send
       an error message back to the client. Note that our return will return us from the setDogName
       function, not just the catch statement. That means we can treat the code below the catch
       as being our "if the try worked"
    */
    console.log(err);
...
const searchDogName = async (req, res) => {
  ...
  */
  ...
  // Increase the dog's age by one
  await Dog.updateOne({ name: doc.name }, { age: doc.age + 1 });
